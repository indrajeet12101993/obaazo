package com.ansh.obaazo.utils;

public class AppConstant {

    public static final String SERVER_URL = "https://obaazo.com/";
    // public static final String SERVER_URL = "http://technowhizzit.com/obaazo/";
    public static final int MAX_WIDTH = 1024;
    public static final int MAX_HEIGHT = 768;
    public static final int size = (int) Math.ceil(Math.sqrt(MAX_WIDTH * MAX_HEIGHT));


    public static final String START_DATE = "start_date";
    public static final String END_DATE = "end_date";
    public static final String NO_OF_ROOM = "room_count";
    public static final String NO_OF_ADULT = "adult_count";
    public static final String NO_OF_CHILD = "child_count";
    public static final String BOOKING_DATES = "booking_details";

    public static final String HOTEL_DETAILS = "hotel_details";
    public static final String BOOK_ROOM = "book_room";
    public static final String HOTEL_ID = "hotel_id";


    public static final String BOOKING_DETAILS = "booking_details_main";

    public static final String B_DATE = "booking_date";
    public static final String S_DATE = "start_date";
    public static final String E_DATE = "end_date";
    public static final String B_LATITUDE = "latitude";
    public static final String B_LONGITUDE = "longitude";
    public static final String B_LOCATION = "booking_location";

    public static final String IMAGES = "images";
    public static final String POSITION = "position";
    public static final String PERSON_DETAILS = "person_details";
    public static final String USER_ID = "user_id";
}

